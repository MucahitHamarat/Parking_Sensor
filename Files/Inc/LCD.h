...
void lcd_delay(void);
void lcd_cmd(char out_char); // komut gösterebiliyoruz , imleci göster veya kapat gibi.
void lcd_char_cp(char out_char);
void lcd_char(unsigned char row, unsigned char column, char out_char); //belirlediğin satır ve sütunu aralarına "," koy, sonra yazılaca olanı.
